...
import logging
from custom_components.eneco_price_rates.types import (
    DayAndNightPrice,
    DayPrice,
    ElectricityPrice,
    NightPrice,
...
def parse_electricity_price_from_pdf(filename: str) -> ElectricityPrice:
    ...
    dayPrice = DayPrice(yearly_fee, to_float(data[0].iloc[1][1]) / 100)
    nightPrice = NightPrice(yearly_fee, to_float(data[0].iloc[1][5]) / 100)
    dayAndNightPrice = DayAndNightPrice(
        yearly_fee,
        to_float(data[0].iloc[1][2]) / 100,
        to_float(data[0].iloc[1][3]) / 100,
    )
