...
class EnecoWeb:
    ...
    async def fetch_electricity_products(self) -> list[EnecoProduct]:
        ...
        for item in soup.select('div.col-lg-7 > div > div:nth-child(1) > div > ul > li > a'):
            products.append(EnecoProduct(item.get_text(), item.get('href')))
        period = soup.select_one('#ewk-app > header > div > div > div > div.header__content > h1').get_text().replace('Tariefkaarten ', '')
